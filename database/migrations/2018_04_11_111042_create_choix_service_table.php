<?php

use Illuminate\Support\Facades\Schema;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Database\Migrations\Migration;

class CreateChoixServiceTable extends Migration
{
    /**
     * Run the migrations.
     *
     * @return void
     */
    public function up()
    {
        Schema::create('Choix_Service', function (Blueprint $table) {
            $table->increments('choix_service_id');

            $table->tinyInteger('choice');
            $table->tinyInteger('real_affect');


            $table->integer('etudiant_id')->unsigned();
            $table->foreign('etudiant_id')->references('etudiant_id')->on('Etudiant');


            $table->integer('service_id')->unsigned();
            $table->foreign('service_id')->references('service_id')->on('Service');


            $table->integer('s_groupe_id')->unsigned();
            $table->foreign('s_groupe_id')->references('s_groupe_id')->on('S_Groupe');


            $table->softDeletes();
            $table->timestamps();
        });
    }

    /**
     * Reverse the migrations.
     *
     * @return void
     */
    public function down()
    {
        Schema::dropIfExists('choix_service');
    }
}
