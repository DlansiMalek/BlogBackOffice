...
class CongressServices
{
    ...
    {
        $accessTable = $participant->accesss;
        if ($participant->pack) {
            if (!$accessTable) $accessTable = [];
            foreach ($participant->pack->accesses as $packAccess) {
                $accessExists = false;
                foreach ($accessTable as $access){
                    if ($access->access_id == $packAccess->access_id){
                        $accessExists = true;
                        break;
                    }
                }
                if (!$accessExists) array_push($accessTable, $packAccess);
            }
        }
        $accesses = "";
        if (sizeof($accessTable) > 0) {
